<x-guest-layout>
    <x-jet-authentication-card>
        ...
        <form method="POST" action="{{ route('register-step2.post') }}" enctype="multipart/form-data">
            ...
            <div>
                <x-jet-label for="phone" value="{{ trans('frontend.profile.content.phone') }}" />
                <x-jet-input id="phone" class="block mt-1 w-full" type="text" name="phone" :value="old('phone')" />
            </div>

            <div class="mt-4">
                <x-jet-label for="address" value="{{ trans('frontend.profile.content.address') }}" />
                <x-jet-input id="address" class="block mt-1 w-full" type="text" name="address" :value="old('address')" />
            </div>

            <div class="mt-4">
                <x-jet-label for="city_id" value="{{ trans('frontend.profile.content.city') }}" />
            ...
            <div class="mt-4">
                <x-jet-label for="photo" value="{{ trans('frontend.profile.content.profile_photo') }}" />
                <input type="file" name="photo" />
            </div>

            <div class="flex items-center justify-end mt-4">
                <x-jet-button class="ml-4">
                    {{ trans('frontend.profile.content.finish_registration') }}
                </x-jet-button>
            </div>
        </form>
    </x-jet-authentication-card>
